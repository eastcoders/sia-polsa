...
class ListBpmi extends Component implements HasActions, HasSchemas, HasTable
{
    ...
    {
        return $table
            ->query(fn(): Builder => User::query()->role('bpmi'))
            ->columns([
            ...
            ->headerActions([
                Action::make('add_bpmi')
                    ->label('Tambah User BPMI')
                    ...
                    ->action(function (array $data) {
                        ...
                        if (!$user->hasRole($role)) {
                            ...
                            Notification::make()->title('Akses BPMI Berhasil Ditambahkan')->success()->send();
                        } else {
                            Notification::make()->title('User ini sudah memiliki akses BPMI')->warning()->send();
                        }
                    })
            ])
            ->recordActions([
                DeleteAction::make()
                    ...
                    ->modalHeading('Hapus Akses BPMI?')
                    ->modalDescription('User ini tidak akan bisa mengakses menu BPMI lagi, namun akun User tetap ada.')
                    ->action(function (User $record) {
                        $record->removeRole('bpmi');
                        Notification::make()->title('Akses BPMI Dihapus')->success()->send();
                    })
            ])
            ->toolbarActions([
                //
    ...
    {
        return view('livewire.bpmi.list-bpmi');
    }
}
