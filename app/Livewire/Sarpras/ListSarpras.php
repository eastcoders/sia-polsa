...
class ListSarpras extends Component implements HasActions, HasSchemas, HasTable
{
    ...
    {
        return $table
            ->query(fn(): Builder => User::query()->role('sarpras'))
            ->columns([
            ...
            ->headerActions([
                Action::make('add_sarpras')
                    ->label('Tambah User Sarpras')
                    ...
                    ->action(function (array $data) {
                        ...
                        if (!$user->hasRole($role)) {
                            ...
                            Notification::make()->title('Akses Sarpras Berhasil Ditambahkan')->success()->send();
                        } else {
                            Notification::make()->title('User ini sudah memiliki akses Sarpras')->warning()->send();
                        }
                    })
            ])
            ->recordActions([
                DeleteAction::make()
                    ...
                    ->modalHeading('Hapus Akses Sarpras?')
                    ->modalDescription('User ini tidak akan bisa mengakses menu Sarpras lagi, namun akun User tetap ada.')
                    ->action(function (User $record) {
                        $record->removeRole('sarpras');
                        Notification::make()->title('Akses Sarpras Dihapus')->success()->send();
                    })
            ])
            ->toolbarActions([
                //
    ...
    {
        return view('livewire.sarpras.list-sarpras');
    }
}
