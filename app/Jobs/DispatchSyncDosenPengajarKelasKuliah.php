...
class DispatchSyncDosenPengajarKelasKuliah implements ShouldQueue
{
    ...
    public function handle(PddiktiClient $client): void
    {
        try {
            // 1. Get Total Data
            $response = $client->getCountDosenPengajarKelasKuliah(['filter' => $this->filter['filter'] ?? '']);
            ...
            if ($totalData === 0) {
                Log::info('No dosen pengajar kelas kuliah data to sync.');
            ...
            for ($offset = 0; $offset < $totalData; $offset += $batchSize) {
                $jobs[] = new SyncDosenPengajarKelasKuliahJob($batchSize, $offset, $this->filter);
            }
            ...
            Log::error('Failed to dispatch sync dosen pengajar kelas kuliah: ' . $e->getMessage());
            throw $e;
        }
    }
}
